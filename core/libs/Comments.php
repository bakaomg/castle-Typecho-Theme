 ...
 */
class Castle_Comments_Archive extends Widget_Abstract_Comments {
 /**
  * 当前页
  *
  * @access private
 ...
 private function threadedCommentsCallback() {
  ...
  if ($this->authorId) {
   if ($this->authorId == $this->ownerId) {
    ...
    $commentOwner .= '<div class="moe-comment-by-owner" mdui-tooltip="{content: \''.$GLOBALS['CastleLang']['comment']['commrntsBox']['blogger'].'\'}"></div>';
   } else {
    ...
    <div id="<?php $this->theId(); ?>" class="moe-comments-box<?php echo $commentClass; ?>">
     <main class="moe-comments-content-wrap">
      ...
      <div class="moe-comments-content">
       <div class="moe-comments-author mdui-typo">
       ...
       <div class="moe-comments-time">
        <?php echo date($GLOBALS['CastleLang']['comment']['commrntsBox']['time'], $this->created); ?>
        <?php if ('waiting' == $this->status) { ?><br><span><?php if(!$this->user->hasLogin()) { echo $GLOBALS['CastleLang']['comment']['commrntsBox']['status']['logout']; }else{ echo $GLOBALS['CastleLang']['comment']['commrntsBox']['status']['login']; } ?></span><?php } ?>
       </div>

       <div class="moe-comments-reply">
...
<?php if ($this->children) { ?>     <div class="moe-comment-children" itemprop="discusses">
 ...
 private function getParent() {
  ...
  if($parentID == '0') {
   ...
   if (!array_key_exists('author', $author) || empty($author['author'])) {
    $author['author'] = $GLOBALS['CastleLang']['comment']['commrntsBox']['delete'];
    $link = '';
   } elseif (!empty($author['author']) && @$author['status'] == 'waiting'){
    //如果是带审核的评论
    if(!$this->user->hasLogin()) {
     $author['author'] = $GLOBALS['CastleLang']['comment']['commrntsBox']['pending'];
     $link = '';
    }else{
   ...
   if (!$link) {
    return '<a class="moe-comment-reply-at" no-go>@'.$author['author'].'</a> ';
   }else{
    return '<a '.$link.' class="moe-comment-reply-at" no-go>@'.$author['author'].'</a> ';
   }
