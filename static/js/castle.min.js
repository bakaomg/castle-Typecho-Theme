...
var CastleCreate = {
 ...
 Sidebar: function() {
  ...
  id.innerHTML = '<div class="moe-sidebar-header" style="background-image: url(\''+CastleConfig.setting.sidebar.background+'\')">\
   ...
   <div class="moe-sidebar-header-siteInfo"><div class="moe-sidebar-header-authorName">ohmyga</div><div class="moe-sidebar-header-description">一只想咸鱼翻身的萌新</div></div>\
  </div>\
  ...
  //为抽屉工具栏按钮绑定事件
  var toolsDom = $$('#sidebar .moe-sidebar-toolbar button');
  var toolsFuncName = ["CastleSetting.panel()", CastleData.login.onclick, "CastleNight.toggle()"];
  for(let i=0; i<toolsDom.length; ++i) {
   toolsDom[i].onclick = function() {
    var scr = document.createElement("script");
    scr.innerHTML = toolsFuncName[i];
    $$('body').append(scr);
    setTimeout(function() {
 ...
 Footer: function() {
  ...
  if (CastleConfig.setting.miibeian) {
   var miibeian = '<div class="moe-footer-icp"><a href="http://www.beian.miit.gov.cn/" target="_blank">'+CastleConfig.setting.miibeian+'</a></div>';
  }else{
   var miibeian = '';
  }
...
var CastleNight = {
 ...
 toggle: function() {
  ...
  if(status) { this.close(); }else{ this.open(); }
 },
 ...
 open: function() {
  ...
  setTimeout(function(){
   ...
   mdui.snackbar({message: CastleLang.sidebar.dark.dark, position: 'right-bottom',onOpen: function() { var snackbar = document.querySelector('.mdui-snackbar'); snackbar.classList.add('mdui-color-blue-600'); }}); //弹出切换成功消息
 ...
 close: function() {
  ...
  setTimeout(function(){
   ...
   mdui.snackbar({message: CastleLang.sidebar.dark.light, position: 'right-bottom'}); //弹出切换成功消息
 ...
 create: function() {
  ...
  for(let owoNum=0; owoNum<owoList.length; ++owoNum) {
   ...
   if(owoList[owoNum].type == 'text') {
    ...
    if (owoList[owoNum].content) {
     for(let i=0; i<owoList[owoNum].content.length; ++i) {
      owoTmps.owo += '<a data-owo="'+owoList[owoNum].content[i].text+'" class="moe-owo-text-btn mdui-btn mdui-shadow-2" no-go no-pjax>'+owoList[owoNum].content[i].text+'</a>';
     }
    }else{
     //如果什么都没有
    ...
    owoTmps.owo += '</div>';
    //加上tab
    owoTmps.tab += '<a href="#'+owoList[owoNum].id+'" class="mdui-ripple" no-go>'+owoList[owoNum].name+'</a>';
   };
   ...
   if(owoList[owoNum].type == 'emoji') {
    ...
    if (owoList[owoNum].content) {
     for(let i=0; i<owoList[owoNum].content.length; ++i) {
      owoTmps.owo += '<a data-owo="'+owoList[owoNum].content[i].text+'" class="moe-owo-emoji-btn mdui-btn mdui-shadow-2" no-go no-pjax>'+owoList[owoNum].content[i].text+'</a>';
     }
    }else{
     //如果什么都没有
     owoTmps.owo += '<div class="moe-owo-main-error">此表情包下并没有任何表情..</div>';
   ...
   if(owoList[owoNum].type == 'smallPicture') {
    ...
    if (owoList[owoNum].content) {
     for(let i=0; i<owoList[owoNum].content.length; ++i) {
      owoTmps.owo += '<a data-owo="'+owoList[owoNum].content[i].data+'" class="moe-owo-smallPicture-btn mdui-btn mdui-shadow-2" no-go no-pjax>\
   ...
   if(owoList[owoNum].type == 'picture') {
    ...
    if (owoList[owoNum].content) {
     for(let i=0; i<owoList[owoNum].content.length; ++i) {
      owoTmps.owo += '<a data-owo="'+owoList[owoNum].content[i].data+'" class="moe-owo-picture-btn mdui-btn mdui-shadow-2" no-go no-pjax>\
       <img src="'+owoList[owoNum].dir+owoList[owoNum].content[i].file+'" />\
      </a>';
     }
    }else{
  ...
  $$.ajax({
   ...
   success: function(data) {
    ...
    if ($$(Obj)[0].querySelector('.container') == null) {
     ...
     if('' === CastleData.comments.replyTo) {
...
var CastleLibs = {
 ...
 scrollClick: function() {
  for (let i=0; i<$$('a[href*="#"]:not([no-pgo])').length; ++i){
   $$('a[href*="#"]:not([no-pgo])')[i].addEventListener('click', function() {
